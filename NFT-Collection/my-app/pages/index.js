...
export default function Home() {
  ...
  const presaleMint = async () => {
    try {
      ...
      // update methods
      const nftContract = new Contract("0x86803E7f303f06a678114630E09cDC7a228b6b8b", abi, signer);
  ...
  const checkIfPresaleEnded = async () => {
    try {
      ...
      // We connect to the Contract using a Provider, so we will only
      // have read-only access to the Contract
      const nftContract = new Contract("0x86803E7f303f06a678114630E09cDC7a228b6b8b", abi, provider);
  ...
  const getProviderOrSigner = async (needSigner = false) => {
    ...
    if (chainId !== 80001) {
      window.alert("Cange the network to polygon");
